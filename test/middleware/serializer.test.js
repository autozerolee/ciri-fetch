import serializer, { paramsSerialize } from '../../src/middleware/serializer';

const next = () => {};

describe('Test Serializer Middleware', () => {
  it('should do nothing when ctx is null', async done => {
    const ctx = null;
    await serializer(ctx, next);
...
describe('Test paramsSerialize:', () => {
  ...
  it('should support nesting Date params', () => {
    let date = new Date();
    expect(paramsSerialize({ a: date })).toBe(`a=${date.valueOf()}`);
  });
  ...
  it('should support nesting array params', () => {
    ...
    expect(paramsSerialize({ a: [1, 2, 3] })).toBe('a=1%2C2%2C3');
  });
